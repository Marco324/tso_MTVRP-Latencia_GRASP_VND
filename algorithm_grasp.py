...
Formatos de instancia soportados:
    ...
    (3) Formato simple (NODES) y CVRP/TSPLIB        - compatibilidad

Autor: (completar con los datos del equipo)
===========================================================================
"""
...
def _capacity_ok(trip, inst):
    return sum(inst.demand[c] for c in trip) <= inst.Q + 1e-9
...
def local_search(solution, inst):
    ...
    while improved:
            ...
            for pi in range(len(sol[ti])):
                ...
                for tj in range(len(sol)):
                    for pj in range(len(sol[tj]) + 1):
                        ...
                        ns[ti].pop(pi)
                        if ti == tj and pj > pi:
                            ns[tj].insert(pj - 1, cust)
                        else:
                            ns[tj].insert(pj, cust)
                        ns = [t for t in ns if t]
                        if not all(_capacity_ok(t, inst) for t in ns):
                            continue
                        cand = optimal_trip_order(ns, inst)
                        val, feas = evaluate(cand, inst)
                        if feas and val < best_val - 1e-7:
                            sol = [list(t) for t in cand]
                            best_val = val
                            improved = True
                            break
                    if improved:
        ...
        # Vecindario 2: swap
        flat = [(ti, pi) for ti in range(len(sol))
                for pi in range(len(sol[ti]))]
        for a in range(len(flat)):
            for b in range(a + 1, len(flat)):
                ...
                tj, pj = flat[b]
                ns = [list(t) for t in sol]
                ns[ti][pi], ns[tj][pj] = ns[tj][pj], ns[ti][pi]
                if not all(_capacity_ok(t, inst) for t in ns):
                    continue
                cand = optimal_trip_order(ns, inst)
                val, feas = evaluate(cand, inst)
