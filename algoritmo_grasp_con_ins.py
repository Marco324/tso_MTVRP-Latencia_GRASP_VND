...
def calcular_incremento_insercion(viaje_actual, cliente, posicion,
                                   instancia):
    """
    Calcula cuanto AUMENTA la latencia interna del viaje si insertamos
    'cliente' en la posicion indicada.

    Insertar un cliente en una posicion afecta a la latencia de:
      (a) El propio cliente nuevo (su tiempo de llegada).
      (b) TODOS los clientes que vienen DESPUES en el mismo viaje,
          porque su tiempo de llegada se desplaza por:
              + (distancia(prev -> cliente) + servicio(cliente)
                 + distancia(cliente -> sig)) - distancia(prev -> sig)
          es decir, el "detour" que provoca insertarlo.

    ENTRADA:
        viaje_actual : lista de clientes ya en el viaje.
        cliente      : el cliente candidato a insertar.
        posicion     : indice donde se insertaria (0..len(viaje_actual)).
        instancia    : objeto Instancia.

    SALIDA:
        float, incremento total de la latencia interna del viaje al
        insertar al cliente en esa posicion. Es siempre >= 0.
    """
    distancias = instancia.matriz_distancias
    servicios = instancia.tiempos_servicio

    # Nodo anterior a la posicion de insercion (deposito si es al inicio).
    if posicion == 0:
        nodo_previo = 0
    else:
        nodo_previo = viaje_actual[posicion - 1]

    # Nodo siguiente (deposito si insertamos al final del viaje).
    if posicion == len(viaje_actual):
        nodo_siguiente = 0
    else:
        nodo_siguiente = viaje_actual[posicion]

    # ----- Calcular el tiempo de llegada al nodo_previo (sin insercion).
    # Asi sabemos cual seria el tiempo de llegada al cliente insertado.
    tiempo_hasta_previo = 0.0
    nodo_anterior = 0
    for i in range(posicion):
        nodo_en_i = viaje_actual[i]
        tiempo_hasta_previo += distancias[nodo_anterior][nodo_en_i]
        tiempo_hasta_previo += servicios[nodo_en_i]
        nodo_anterior = nodo_en_i
    # En este punto, tiempo_hasta_previo = tiempo justo despues de
    # terminar el servicio del nodo_previo (o 0 si posicion == 0).

    # ----- Contribucion (a): latencia del cliente recien insertado.
    llegada_al_cliente_nuevo = (tiempo_hasta_previo
                                + distancias[nodo_previo][cliente])

    # ----- Contribucion (b): desplazamiento que sufren todos los
    # clientes que estan DESPUES de la posicion de insercion.
    # El detour adicional que se agrega a sus tiempos de llegada es:
    detour = (distancias[nodo_previo][cliente]
              + servicios[cliente]
              + distancias[cliente][nodo_siguiente]
              - distancias[nodo_previo][nodo_siguiente])
    numero_clientes_desplazados = len(viaje_actual) - posicion

    incremento_total = (llegada_al_cliente_nuevo
                        + detour * numero_clientes_desplazados)
...
def construir_solucion_grasp(instancia, alpha, generador_aleatorio,
    ...
    DOS MODOS DE CONSTRUCCION SOPORTADOS:

        "vecino_mas_cercano" (alias: "nn")
            - Solo decide QUE cliente agregar.
            - Lo coloca SIEMPRE al final del viaje en construccion.
            - Puntuacion = distancia desde el nodo actual al candidato.
            - Es la heuristica clasica de "nearest neighbor".

        "menor_incremento" (alias: "ins")
            - Decide QUE cliente agregar Y EN QUE POSICION del viaje.
            - Para cada candidato y cada posicion posible, calcula el
              incremento de latencia interna del viaje que provoca esa
              insercion. Se queda con la mejor posicion para cada
              candidato.
            - Puntuacion = mejor incremento posible.
            - Es mas costoso (O(n^2)) pero da soluciones iniciales muy
              superiores cuando la latencia importa (no solo la distancia).

    COMO SE ARMA LA RCL (para cada paso):
        - Se calcula la "puntuacion" de cada candidato (segun el modo).
    ...
    ENTRADA:
        ...
        modo                 : str, "vecino_mas_cercano" o "menor_incremento"
                               (tambien se aceptan "nn" e "ins").
    ...
    # Normalizar el nombre del modo para aceptar alias cortos.
    if modo in ("nn", "vecino_mas_cercano"):
        modo_efectivo = "vecino_mas_cercano"
    elif modo in ("ins", "menor_incremento"):
        modo_efectivo = "menor_incremento"
    else:
        raise ValueError(f"Modo de construccion desconocido: {modo}")
    ...
    while clientes_sin_visitar:
        ...
        while True:
            ...
            # ---------------------------------------------------------
            # PASO 1: calcular la puntuacion de cada candidato segun el
            # modo elegido. En el modo "menor_incremento" tambien
            # guardamos la MEJOR POSICION de insercion para cada candidato,
            # porque la necesitaremos cuando insertemos al elegido.
            # ---------------------------------------------------------
            if modo_efectivo == "vecino_mas_cercano":
                # Puntuacion simple: distancia desde el nodo actual.
                candidatos_puntuados = sorted(
                    (distancias[nodo_actual][cliente], cliente)
                    for cliente in candidatos
                )
                # En este modo no necesitamos guardar posiciones.
                mejor_posicion_por_candidato = None

            else:  # modo_efectivo == "menor_incremento"
                # Para cada candidato, probar TODAS las posiciones de
                # insercion (0 hasta len(viaje_actual)) y quedarse con la
                # de menor incremento de latencia.
                mejor_posicion_por_candidato = {}
                puntuaciones_con_cliente = []
                for cliente in candidatos:
                    mejor_incremento = float("inf")
                    mejor_posicion = 0
                    for posicion in range(len(viaje_actual) + 1):
                        incremento = calcular_incremento_insercion(
                            viaje_actual, cliente, posicion, instancia)
                        if incremento < mejor_incremento:
                            mejor_incremento = incremento
                            mejor_posicion = posicion
                    mejor_posicion_por_candidato[cliente] = mejor_posicion
                    puntuaciones_con_cliente.append(
                        (mejor_incremento, cliente))
                candidatos_puntuados = sorted(puntuaciones_con_cliente)

            # ---------------------------------------------------------
            # PASO 2: construir la RCL con el umbral parametrizado por alpha.
            # ---------------------------------------------------------
            ...
            # ---------------------------------------------------------
            # PASO 3: elegir un cliente al azar de la RCL e insertarlo.
            # ---------------------------------------------------------
            cliente_elegido = generador_aleatorio.choice(lista_rcl)

            if modo_efectivo == "vecino_mas_cercano":
                # Siempre se inserta al final del viaje.
                viaje_actual.append(cliente_elegido)
            else:
                # Se inserta en la mejor posicion calculada para ese cliente.
                posicion_a_insertar = mejor_posicion_por_candidato[
                    cliente_elegido]
                viaje_actual.insert(posicion_a_insertar, cliente_elegido)

            # Actualizar estado del viaje.
            clientes_sin_visitar.discard(cliente_elegido)
            capacidad_restante -= instancia.demandas[cliente_elegido]
            # nodo_actual solo es relevante para "nn"; para "ins" no se usa
            # directamente, pero lo mantenemos por consistencia.
...
CONFIGURACIONES_POR_DEFECTO = [
    ...
    {"name": "C3-nn-a03", "mode": "vecino_mas_cercano",
